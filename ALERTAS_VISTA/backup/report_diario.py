...
if datetime.now().isoweekday() == 6:
    query_crs_alerta = """SELECT name, telegram_id , cr_list FROM Vista_Proc.dbo.VISTA_USER_ALERTS where alerta3 = 1 and shift_id in (select id from Vista_Proc.dbo.VISTA_SHIFTS vs where sabado = 1 and CONVERT(TIME, GETDATE()) BETWEEN DATEADD(MINUTE,-30,init_date) and DATEADD(MINUTE,30,init_date))"""
#   SE FOR DOMINGO
elif datetime.now().isoweekday() == 7:
    query_crs_alerta = """SELECT name, telegram_id , cr_list FROM Vista_Proc.dbo.VISTA_USER_ALERTS where alerta3 = 1 and shift_id in (select id from Vista_Proc.dbo.VISTA_SHIFTS vs where domingo = 1 and CONVERT(TIME, GETDATE()) BETWEEN DATEADD(MINUTE,-30,init_date) and DATEADD(MINUTE,30,init_date))"""
else:
    query_crs_alerta = """SELECT name, telegram_id , cr_list FROM Vista_Proc.dbo.VISTA_USER_ALERTS where alerta3 = 1 and shift_id in (select id from Vista_Proc.dbo.VISTA_SHIFTS vs where CONVERT(TIME, GETDATE()) BETWEEN DATEADD(MINUTE,-30,init_date) and DATEADD(MINUTE,30,init_date))"""
