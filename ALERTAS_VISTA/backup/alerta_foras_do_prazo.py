...
#token do bot
token = '5630811043:AAG4H48q3m6WtTDZYgmy4FkUIDrdxU9W1fo'
...
{categoria}:  {qtd} tarefas fora do prazo.
 """
# query_crs_alerta = 'SELECT name, telegram_id , cr_list, alerta1 FROM Vista_Proc.dbo.VISTA_USER_ALERTS where alerta1 = 1 and shift_id in (select Id from Vista_Proc.dbo.VISTA_SHIFTS where (init_date > end_date AND (CONVERT(TIME, GETDATE()) > DATEADD(MINUTE,-5,init_date) OR  CONVERT(TIME, GETDATE()) < DATEADD(MINUTE,5,end_date))) or ( CONVERT(TIME, GETDATE()) BETWEEN DATEADD(MINUTE,-5,init_date) and DATEADD(MINUTE,5,end_date) ) )'

if datetime.now().isoweekday() == 6:
    query_crs_alerta = 'SELECT name, telegram_id , cr_list, alerta1 FROM Vista_Proc.dbo.VISTA_USER_ALERTS where alerta1 = 1 and shift_id in (select Id from Vista_Proc.dbo.VISTA_SHIFTS where sabado = 1 and ((init_date > end_date AND (CONVERT(TIME, GETDATE()) >= DATEADD(MINUTE,-5,init_date) OR CONVERT(TIME, GETDATE()) <= DATEADD(MINUTE,5,end_date))) or init_date < end_date and (CONVERT(TIME, GETDATE()) BETWEEN DATEADD(MINUTE,-5,init_date) and DATEADD(MINUTE,5,end_date) )))'
elif datetime.now().isoweekday() == 7:
    query_crs_alerta = 'SELECT name, telegram_id , cr_list, alerta1 FROM Vista_Proc.dbo.VISTA_USER_ALERTS where alerta1 = 1 and shift_id in (select Id from Vista_Proc.dbo.VISTA_SHIFTS where domingo = 1 and ((init_date > end_date AND (CONVERT(TIME, GETDATE()) >= DATEADD(MINUTE,-5,init_date) OR CONVERT(TIME, GETDATE()) <= DATEADD(MINUTE,5,end_date))) or init_date < end_date and (CONVERT(TIME, GETDATE()) BETWEEN DATEADD(MINUTE,-5,init_date) and DATEADD(MINUTE,5,end_date) )))'
else:
...
df_crs = exec_query(query_crs_alerta)
if df_crs.empty:
    exit()
...
#query
query = """select COUNT(Id) as qtd, ServicoDescricao, t.EstruturaGrupo, Nivel_03 as CR from Vista.dbo.Tarefa t
inner join DW_Vista.dbo.DM_ESTRUTURA de on de.Id_Estrutura = t.EstruturaId 
where TRY_CAST(LEFT(Nivel_03,6) AS INT) IN ({}) and TerminoReal  is null and Prazo < getdate() 	
